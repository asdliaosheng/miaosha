...
public class OrderServiceImpl implements OrderService {
    ...
    //创建订单
    @Override
    public ResultVO create(String userId, String shippingId) {
        //取购物车cartDTO对象,一个用户只有一个购物车主表,复用
        CartDTO cartDTO = cartClient.search(userId);
        ...
        if(!StringUtils.isEmpty(lowProductId)){
            return ResultVO.error(CodeMsgVO.PRODUCT_STOCK_NOT_ENOUGH, lowProductId);
        }
        ...
        //删除购物车选定的商品
        cartClient.deleteChecked(userId);
